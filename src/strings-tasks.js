...
function removeFirstOccurrences(str, value) {
  ...
  const lastIndex = firstIndex + value.length - 1 + 1;
  const newStr = str.slice(0, firstIndex) + str.slice(lastIndex, str.length);
  if (firstIndex === -1) {
    return str;
  }
  return newStr;
...
function orderAlphabetically(str) {
  const endStr = str.split('').sort().join('');
  return endStr;
}
...
function isPalindrome(str) {
  const strN = str
    ...
    .split(/[\s!.?,;:'"-()]/gi)
    .join('');
  const revStr = strN.split('').reverse().join('');
  if (strN === revStr) {
    return true;
  }
  return false;
...
function reverseWords(str) {
  const arr = str.split(' ');
  const arrN = arr.map(function revEl(el) {
    const el1 = el.split('').reverse().join('');
    return el1;
  });
  return arrN.join(' ');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    if (arr[i].match(/[a-zA-Z]/g) && arr[i] === arr[i].toLowerCase()) {
      arrN.push(arr[i].toUpperCase());
    }
    if (arr[i].match(/[a-zA-Z]/g) && arr[i] === arr[i].toUpperCase()) {
      arrN.push(arr[i].toLowerCase());
    }
  }
  return arrN.join('');
}
...
function encodeToRot13(str) {
  ...
  const strFin = str.replace(
    /[a-zA-Z]/gi,
    (str1) => alphaRot[alpha.indexOf(str1)]
  );
  return strFin;
}
